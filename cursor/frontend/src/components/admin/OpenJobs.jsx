...
const OpenJobs = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      setError(error.message || 'Failed to load open jobs')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="p-6 overflow-y-auto max-h-[70vh]">
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-blue-600">
                This job is open and waiting for an employee to accept it. Once an employee starts working on it, 
    ...
    <div className="p-6 md:p-8 lg:p-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-text-primary mb-2">Open Jobs</h1>
            <p className="text-text-secondary">View and manage jobs waiting for assignment</p>
          </div>
          <div className="mt-4 md:mt-0 flex items-center gap-4">
            <div className="bg-blue-50 px-4 py-2 rounded-lg border border-blue-200">
              <span className="text-2xl font-bold text-blue-700">{filteredJobs.length}</span>
              <span className="text-sm text-blue-600 ml-2">Open</span>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <EmptyState 
            icon="job" 
            title="No Open Jobs" 
            description="There are no jobs currently waiting for assignment." 
          />
        )}
